...
(defadvice! +robbert--strip-project-root-a (fn &rest args)
  ...
  (let ((buffer-path (apply fn args))
        (root (abbreviate-file-name (or (marginalia--project-root) ""))))

    ;; For readability's sake, we want to strip both the current project's root,
    ;; and also abbreviate leading directories in long paths.
    (shrink-path-file (string-remove-prefix root buffer-path))))
#+end_src
