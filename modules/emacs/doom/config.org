...
(map!
 ...
 :nv "M-f"         #'+default/search-buffer
 :nv "M-F"         #'+default/search-project
...
(use-package! copilot
  ...
  ;; :hook (prog-mode . copilot-nes-mode)
...
(after! copilot-nes
  (map!
   :map copilot-nes-mode-map
   :n [tab] `(menu-item "" copilot-nes-accept
             :filter ,(lambda (cmd)
...
Fix multi-line NES insertion overlays not displaying correctly.

Root cause: =copilot-nes--display= sets =evaporate t= on the zero-length insertion
overlay. Per the Emacs manual, setting a non-nil evaporate property on an *empty*
overlay via =overlay-put= causes it to be deleted immediately — before it is ever
rendered. The non-empty deletion overlay is unaffected. The fix removes =evaporate=
from the insertion overlay; =copilot-nes--clear= handles explicit cleanup anyway.

As a secondary fix, when the deleted region ends at the start of a line, a trailing
newline is added to the insertion text so the next undeleted buffer line is not
merged onto the last insertion line.

#+begin_src emacs-lisp
(after! copilot-nes
  (defadvice! +robbert--copilot-nes-fix-display (edit)
    "Fix NES insertion overlay being immediately deleted.
Setting evaporate on an already-empty (zero-length) overlay via overlay-put
causes Emacs to delete the overlay immediately, before it is ever rendered.
The deletion overlay is unaffected because it is non-empty (beg < end).
Also adds a trailing newline when end is at BOL so the next undeleted line
is not merged onto the last insertion line.

See URL `https://github.com/copilot-emacs/copilot.el/issues/451'."
    :override #'copilot-nes--display
    (copilot-nes--clear)
    ...
    (copilot--dbind (text range) edit
      (let* ((region (copilot-nes--range-to-region range))
        ...
        ;; Insertion overlay: show new text as after-string.
        ;; When `end' is at the start of a line, add a trailing newline so
        ;; the first undeleted line is not merged onto the last insertion line.
        (when has-insertion
          (let* ((needs-nl (and (save-excursion (goto-char end) (bolp))
                                (not (string-suffix-p "\n" text))))
                 (display-text (propertize (if needs-nl (concat text "\n") text)
                                           'face 'copilot-nes-insertion-face))
                 (ov (make-overlay end end nil nil nil)))
            (overlay-put ov 'after-string display-text)
            (overlay-put ov 'copilot-nes t)
            ;; Do NOT set evaporate on this zero-length overlay: per the Emacs
            ;; manual, overlay-put of a non-nil evaporate on an already-empty
            ;; overlay deletes it immediately.  copilot-nes--clear handles cleanup.
            (overlay-put ov 'priority 100)
            (push ov copilot-nes--overlays)))))
...
(use-package! highlight-numbers
  :hook ((prog-mode conf-mode yaml-mode) . highlight-numbers-mode)
...
** Magit and diffing
#+begin_src emacs-lisp
;; The default is 4, Doom sets this to 5, but some more advanced options are
;; hidden unless hits is set to 6 or 7
(after! magit
...
(after! csharp-mode
  (set-electric! 'csharp-ts-mode :chars '(?\n ?\{)))
#+end_src
...
(setq-hook! '(c-mode-hook c-ts-mode-hook c++-mode-hook c++-ts-mode-hook) c-basic-offset 4)
(setq c-ts-mode-indent-offset 4)

;; Match the Chromium clang-format style
(add-hook! '(c-mode-hook c-ts-mode-hook c++-ts-mode-hook) :append
...
(add-to-list '+format-on-save-disabled-modes 'haskell-ts-mode t)
...
(after! haskell-mode
  ...
  ;; HLS is supposed to have hlint integration, but it hasn't been working for a
  ;; while now
  (after! lsp-mode
    (set-next-checker! 'haskell-mode 'lsp 'haskell-hlint))
  (flycheck-add-mode 'haskell-hlint 'haskell-mode)
...
(after! apheleia
  ...
  (add-to-list 'apheleia-mode-alist '(markdown-ts-mode . prettier)))
...
#+begin_src emacs-lisp
(setq-hook! 'python-ts-mode-hook fill-column 79)

(add-to-list 'auto-mode-alist '("Pipfile$" . conf-toml-mode))
(add-to-list 'auto-mode-alist '("Pipfile\\.lock$" . json-mode))

(setq ;; Otherwise it will try to load every single Python project it knows of
      ;; instead of just the one we're working on
      lsp-pyright-multi-root nil
      lsp-pyright-langserver-command "basedpyright")

(after! lsp-ruff
  ;; FIXME: Ruff fails to start without this. Should be temporary.
  (add-to-list 'lsp-ruff-ruff-args "--preview"))

;; `lsp-mode` requests semantic tokens before Ruff has finished initializing,
;; causing Ruff to terminate. Either semantic tokens need to be disabled, or
;; Ruff does.
(setq-hook! 'python-ts-mode-hook lsp-semantic-tokens-enable nil)
...
(after! python
  ...
  ;; (set-electric! 'python-ts-mode ':words '("else:"))
  ;; ;; FIXME: The above doesn't clear the list of electric indent characters
  ;; ;;        anymore
  ;; (setq-hook! 'python-ts-mode-hook electric-indent-chars '())
...
(after! web-mode
  ...
  ;; of classes)
  (set-electric! 'web-ts-mode :chars '(?\<) :words '("endfor" "endif" "endblock"))
